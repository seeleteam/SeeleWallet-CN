...
function ToAccountInfo(publickey, balance, shard) {
    ...
    divhtml += `<ul>`;
    divhtml += `<li>账户</li>`;
    divhtml += `<li><span>` + balance + `</span> 元一</li>`;
    divhtml += `<li class="publickey">` + publickey + `</li>`;
    divhtml += `<li class="shard">` + "片-" + shard + `</li>`;
    divhtml += `</ul>`;
    divhtml += `</div>`;
    divhtml += `</div>`;
...
//     divhtml += `</dl>`;
    ...
    divhtml += `<dd id="qr_request">二维码</dd></br>`;
    divhtml += `</dl>`;
    divhtml += `<dl id="qr_result" class="qr_result" align="left">`
    divhtml += `</dl>`
    divhtml += `</div>`;
    divhtml += `<h1 class="note">注释</h1>`;
    divhtml += `<p class="info">账户是被密码锁保护的公私钥。他们能控制合约但不能直接显示 <span>交易</span>.</p>`;
...
function showQR (publickey) {
    ...
    if(result.style.display !== 'flex') {
        ...
        request.innerText = "隐藏二维码";
    } else {
        result.style.display = 'none';
        request.innerText = "显示二维码";
        result.innerHTML = "";
    }
}
