...
function loadAccount() {
    ...
    tabs1HTML += `<div><button class="export-account" title="EXPORT ACCOUNTS" onclick="exportAccounts()">导出</button>`
    tabs1HTML += `<button class="import-account" title="IMPORT ACCOUNTS" onclick="importAccounts()">导入</button></div>`
    tabs1HTML += `</div>`
    ...
    tabs1HTML += `<span>创建账户</span>`
    tabs1HTML += `</button>`
    tabs1HTML += `<p class="info">账户指的是用于签名交易与合约所用的钥匙文本身不能展示账户交易</p>`
    tabs1HTML += `<h3 class="latest-title">最新交易记录</h3>`
    ...
    for(var item in seeleClient.txArray) {
        ...
        tabs1HTML += `</p>`
        tabs1HTML += `<ul class="contact-right"><li>创建</li>`
    ...
    for (var item in seeleClient.accountArray) {
        seeleClient.getBalance(seeleClient.accountArray[item].trim(), function (info, err) {
            if (err) {
                ...
                accountHTML += `<li><span class="accountBalance">` + "0" + `</span> 元一</li>`;
                ...
                accountHTML += `<span class="accountImg"><img src="./src/img/Headportrait.png"></span>`;
                accountHTML += `<ul>`;
                accountHTML += `<li>账户</li>`;
                accountHTML += `<li><span class="accountBalance">` + (info.Balance / 100000000).toFixed(3) + `</span> 元一</li>`;
                accountHTML += `<li>` + info.Account + `</li>`;
                accountHTML += `<li>` + "片Shard-" + seeleClient.getShardNum(info.Account) + `</li>`;
                accountHTML += `</ul>`;
                accountHTML += `</div>`;
                accountlist.innerHTML += accountHTML;
                if (count == 0) {
